...
def run_all_classifiers(dataset):
    ...
    print(params_perc[0].reshape(-1))
    predict = perc.test(dataset.train_X, params_perc[0])
    evaluation = perc.evaluate(predict, dataset.train_y)
    predict2 = perc.test(dataset.test_X, params_perc[0])
    evaluation2 = perc.evaluate(predict2, dataset.test_y)
    print("Accuracy train: %f test: %f" % (evaluation, evaluation2))
    fig, axis = dataset.add_line(fig, axis, params_perc[0], "Perceptron", "blue")
